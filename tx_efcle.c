...
REG_1   TX_EVENT_FLAGS_GROUP *group_ptr;    /* Working group pointer  */

    
    /* Setup pointer to event flags control block.  */
    group_ptr =  (TX_EVENT_FLAGS_GROUP *) thread_ptr -> tx_suspend_control_block;

    /* Disable interrupts to remove the suspended thread from the event flags group.  */
    TX_DISABLE

    /* Determine if the cleanup is still required.  */
    if ((thread_ptr -> tx_suspend_cleanup) && (group_ptr) &&
        (group_ptr -> tx_event_flags_id == TX_EVENT_FLAGS_ID))
    {
        ...
        else
        {
            ...
            group_ptr -> tx_event_flags_suspension_list =  thread_ptr -> tx_suspended_next;
        ...
        /* Now we need to determine if this cleanup is from a terminate or a 
           timeout.  */
        if (thread_ptr -> tx_state == TX_EVENT_FLAG)
        {

            /* Thread still suspended on the event flags group.  Setup return error 
               status and resume the thread.  */

            /* Setup return status.  */
            thread_ptr -> tx_suspend_status =  TX_NO_EVENTS;
