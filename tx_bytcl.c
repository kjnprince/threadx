...
REG_1   TX_BYTE_POOL *pool_ptr;            /* Working byte pool pointer  */

    
    /* Setup pointer to byte pool control block.  */
    pool_ptr =  (TX_BYTE_POOL *) thread_ptr -> tx_suspend_control_block;
        ...
        (pool_ptr -> tx_byte_pool_id == TX_BYTE_POOL_ID))
    {
        ...
        else
        {
            ...
            pool_ptr -> tx_byte_pool_suspension_list =  thread_ptr -> tx_suspended_next;
        ...
        /* Now we need to determine if this cleanup is from a terminate or a 
           timeout.  */
        if (thread_ptr -> tx_state == TX_BYTE_MEMORY)
        {

            /* Thread still suspended on the byte pool.  Setup return error status and
               resume the thread.  */
